  ...
  return (
    <SafeAreaView edges={["top"]} className="flex-1 bg-[#020617]">
      ...
      >
        ...
        {/* =========================================== TAB PICKER */}
        <View
          className="mx-5 mb-4 flex-row rounded-full border border-blue-500/30 bg-white/[0.03] p-1"
          style={{ gap: 6 }}
        >
          {(
            [
        ...
        {/* =========================================== CHALLENGES (Rank A/S) */}
        {tab === "challenges" ? (
          <ChallengesTab profile={profile} claimChallenge={claimChallenge} />
        ) : null}
        ...
        {/* =========================================== SECRETS */}
        {tab === "secret" ? (
          <SecretsTab completedSecretQuests={profile.completedSecretQuests} />
        ) : null}
      </ScrollView>
    </SafeAreaView>
  );
...
function formatProgress(q: Quest): string {
  switch (q.type) {
    ...
    case "muscle_volume": {
      ...
      const p =
        q.progress >= 1000
          ? (q.progress / 1000).toFixed(1) + "t"
          : Math.round(q.progress) + ` ${unit}`;
      const t =
        q.target >= 1000
          ? (q.target / 1000).toFixed(1) + "t"
          : q.target + ` ${unit}`;
      return `${p} / ${t}`;
    }
    case "total_reps":
          ...
          return (
            ...
            >
              ...
              <View className="mt-1 flex-row items-center justify-between">
                <Text className="text-[10px] text-slate-500">
                  {Math.round(prog).toLocaleString()} /{" "}
                  {ch.target.toLocaleString()} {ch.unit}
                </Text>
                {done ? (
                  <Pressable
  ...
  return (
    <View className="px-5 pb-10">
      <View className="flex-row items-end justify-between">
        ...
        <Text className="text-[10px] uppercase tracking-widest text-slate-500">
          {completedSecretQuests.length} découvertes
        </Text>
      </View>

      <Text className="mt-3 text-center text-[11px] italic leading-relaxed text-slate-500">
        « Les quêtes secrètes n'apparaissent au Système qu'une fois déclenchées.
        Entraîne-toi, explore des limites, et la Vérité se révèlera d'elle-même…
        »
      </Text>

      {completedSecretQuests.length === 0 ? (
